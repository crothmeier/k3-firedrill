...
| Disproven | 2026-09-02 attempt 1 (P3, note 1): k3s regenerated `server/token` from `K3S_TOKEN=` in `k3s.service.env` at start (mtime 07:02:28Z, OLD content), and server-2 crash-looped on the runbook's two pinned strings. This is the failure mode the runbook exists to prevent, reproduced on the first real rotation. |
